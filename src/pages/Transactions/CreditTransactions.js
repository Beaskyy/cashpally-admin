...
const CreditTransactions = () => {
  ...
  const getCreditTransactions = async () => {
    ...
    const response = await fetch(`${baseURL}/api/admin/transaction/credit`, options)
    const creditTransactions = await response.json();
    setCreditTransactions(creditTransactions.data)
    setLoading(false)
    console.log(creditTransactions.data)
  }
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <MetaTags>
          <title>Credit Transactions | Cashpally Admin</title>
        </MetaTags>
        <div className="container-fluid">
          <Breadcrumbs maintitle="Cashpally" title="Transactions" breadcrumbItem="Credit Transactions" />
          
          <Row>
            <Col lg={12}>
              <Card>
                <CardBody>
                  <CardTitle className="h4">Credit Transactions</CardTitle>
                  {
                    loading ? (
